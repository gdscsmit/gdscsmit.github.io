  ...
  return (
    <>
      ...
      <div className="blogPage">
        <div id="endcontain" className="container-fluid text-center position-sticky" style={{ backgroundColor: 'white', position: 'sticky', marginBottom: '5rem' }}>
          <strong>
            <font size="7">
          ...
          <ul className='my-5'>
            {blogs.map((blog) => (
              <li key={blog.id} className='list-unstyled mx-auto w-75' style={{ marginBottom: '3rem' }}>
                <div className="card text-center my-5">
                  <div className="card-header h4">
                    <b>Penned by {blog.author.displayName}</b>
                  </div>
                  <div className="card-body">
                    <h5 className="card-title my-4 display-6"><b>{blog.title}</b></h5>
                    <p className="card-text" dangerouslySetInnerHTML={{ __html: blog.content }} />
                    <a href={blog.url} target="_blank" className="btn btn-primary">Visit Blogger</a>
                  </div>
